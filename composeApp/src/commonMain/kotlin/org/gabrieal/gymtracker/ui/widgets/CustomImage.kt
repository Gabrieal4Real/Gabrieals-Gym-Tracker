...
fun AnimatedImage(showImage: Boolean, resource: DrawableResource, isLeft: Boolean): Boolean {
    ...
    var enterAnimation = slideInHorizontally(animationSpec = tween(1000, easing = FastOutSlowInEasing)) { fullWidth -> fullWidth } + fadeIn()
    var exitAnimation = slideOutHorizontally(animationSpec = tween(1000, easing = FastOutSlowInEasing)) { fullWidth -> fullWidth } + fadeOut()

    if (isLeft) {
        enterAnimation = slideInHorizontally(animationSpec = tween(1000, easing = FastOutSlowInEasing)) { fullWidth -> -fullWidth } + fadeIn()
